...
MD::takeOver(void) {
	...
	string str;
	int loop=0;
	int Nion=vars->effectiveIn[0][0].inAtoms.size();
	...
	while(getline(stream,str)) {
		...
		if(loop<Nion){
			vars->effectiveIn[0][0].inAtoms[loop].qx=reading[1];
			vars->effectiveIn[0][0].inAtoms[loop].qy=reading[2];
			vars->effectiveIn[0][0].inAtoms[loop].qz=reading[3];
			vars->effectiveIn[0][0].inAtoms[loop].px=reading[4];
			vars->effectiveIn[0][0].inAtoms[loop].py=reading[5];
			vars->effectiveIn[0][0].inAtoms[loop].pz=reading[6];
		}
		else{
