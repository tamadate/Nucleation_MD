...
MD::boundary_scaling_gas_move(void){
	...
	for (auto &a : vars->CG[1]){
		...
		if (a.qx < vars->CG[0][0].qx-HL) a.qx += d_size, flagx--, flag++;
		if (a.qy < vars->CG[0][0].qy-HL) a.qy += d_size, flagy--, flag++;
		if (a.qz < vars->CG[0][0].qz-HL) a.qz += d_size, flagz--, flag++;
		if (a.qx > vars->CG[0][0].qx+HL) a.qx -= d_size, flagx++, flag++;
		if (a.qy > vars->CG[0][0].qy+HL) a.qy -= d_size, flagy++, flag++;
		if (a.qz > vars->CG[0][0].qz+HL) a.qz -= d_size, flagz++, flag++;
		if (flag>0) {
...
MD::boundary_scaling_ion_move(void){
	...
	for (auto &a : vars->CG[2]){
		flag=flagx=flagy=flagz=0;
		if (a.qx < vars->CG[0][0].qx-HL) a.qx += d_size, flag++;
		if (a.qy < vars->CG[0][0].qy-HL) a.qy += d_size, flag++;
		if (a.qz < vars->CG[0][0].qz-HL) a.qz += d_size, flag++;
		if (a.qx > vars->CG[0][0].qx+HL) a.qx -= d_size, flag++;
		if (a.qy > vars->CG[0][0].qy+HL) a.qy -= d_size, flag++;
		if (a.qz > vars->CG[0][0].qz+HL) a.qz -= d_size, flag++;
		if (flag>0) {
