  ...
  normal_distribution<> distgas(0.0, sqrt(kb*T/pp->mgas));
	...
	while(breakFlag==0){
		...
		for(auto &gin : vars->AA[1]){
			...
			if(r2<25){
				dx=gasOut.qx-vars->AA[0][0].qx;
				dy=gasOut.qy-vars->AA[0][0].qy;
				dz=gasOut.qz-vars->AA[0][0].qz;
  ...
  double c=r(mt)*2*M_PI;
	...
	while(breakFlag==0){
		...
		for(auto &gin : vars->AA[1]){
			...
			double r2=dx*dx+dy*dy+dz*dz;
			if(r2<25){
				dx=vapOut.qx-vars->AA[0][0].qx;
				dy=vapOut.qy-vars->AA[0][0].qy;
				dz=vapOut.qz-vars->AA[0][0].qz;
				r2=dx*dx+dy*dy+dz*dz;
				double rinv=1/sqrt(r2);
				vapOut.qx-=dx*rinv;
				vapOut.qy-=dy*rinv;
