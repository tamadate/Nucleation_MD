...
PotentialAMBER::computeBond(Variables *vars, FLAG *flags) {
	...
	for (auto &bond : vars->bonds) {
		...
		if(flags->eflag) vars->U.Uion+=rk*dr;
	}
}
...
/*intra-molecular interaction (angle)*/
	...
	for (auto angle : vars->angles) {
  	...
  	ions[j].fx -= f1[0] + f3[0];
		...
		ions[k].fy += f3[1];
		ions[k].fz += f3[2];
    if (flags->eflag) vars->U.Uion+= tk*dtheta;
	}
}
...
/*intra-molecular interaction (dihedral)*/
	...
	for (auto &dihedral : vars->dihedrals) {
		double ff2[3],ff4[3],ff1[3],ff3[3];

		int i=dihedral.atom1;
		int j=dihedral.atom2;
		int k=dihedral.atom3;
		int l=dihedral.atom4;
		int type=dihedral.type;
		...
		for(int JJ=0;JJ<dtypes[type].multi;JJ++){
			...
			if (flags->eflag) vars->U.Uion+= dtypes[type].coeff[JJ5] * p_;
		}
